...
| 패키지 | 내용 |
| --- | --- |
| `cesium-drawing` | 코어. `Drawer`, `MeasureController`, `VertexEditor`, geometry 함수 |
| `cesium-drawing-react` | 코어 클래스를 감싼 React hooks |
| `cesium-drawing-adapter` | 라벨·툴팁 등 기본 UI 어댑터 (선택) |

`cesium` 은 peer dependency 다.

```bash
pnpm add cesium-drawing cesium
# React 사용 시
pnpm add cesium-drawing-react react
```
...
```ts
import { VertexEditor } from "cesium-drawing";
