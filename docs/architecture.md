...
서브패스 export:
- `@alz/cesium-drawing` — 메인 (`Drawer`, `MeasureController`, `VertexEditor`, 타입)
- `@alz/cesium-drawing/core` — `InputBus`, `OverlayHost`, `pickCartesian3`
- `@alz/cesium-drawing/geometry` — `getDistance`, `getArea`, `getSlope` 등 순수 함수
...
| cesium-extends | alz/cesium-drawing |
| --- | --- |
...
8. **Step 8 — xitecloud-fe 어댑터**: `ExtendsCesiumViewer` 내부 구현을 새 모듈로 교체 (외부 시그니처 보존)
